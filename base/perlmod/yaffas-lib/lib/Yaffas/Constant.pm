...
sub get_os {
	...
	switch ($lsb_release) {
		case qr/CentOS 6|RedHatEnterpriseServer 6|Scientific 6|OracleServer 6/ { return "RHEL6"; }
		case qr/CentOS 5|RedHatEnterpriseServer 5|Scientific 5/ { return "RHEL5"; }
		case qr/Ubuntu/ { return "Ubuntu"; }
		case qr/Debian/ { return "Debian"; }
		else { return "unknown"; }
	}
}
...
use constant {
	DIR => {
		...
		"ldap_schema" => (OS eq 'Ubuntu' or OS eq 'Debian') ? "/etc/ldap/schema/" : "/etc/openldap/schema/",
...
use constant {
	FILE => {
		...
		"slapd_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian') ? "/etc/ldap/slapd.conf" : "/etc/openldap/slapd.conf",
		"slapd_default_conf" => "/etc/default/slapd",
		"libnss_ldap_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian') ? "/etc/libnss-ldap.conf" : "/etc/ldap.conf",
		"pam_ldap_conf" => "/etc/pam_ldap.conf",
		"ldap_conf" => "/etc/ldap.conf",
		"ldap_ldap_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/ldap/ldap.conf" : "/etc/openldap/ldap.conf",
		...
		"exim_passwd_client" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/exim4/passwd.client" : "/etc/exim/passwd.client",
		"hosts" => "/etc/hosts",
...
#		"smb_includes_global" => ( OS eq 'Ubuntu' or OS eq 'Debian' ? "/etc/samba/smbopts.global" : "/etc/samba/smb.conf" ),
		...
		"mysql_cnf" => ( OS eq 'Ubuntu' or OS eq 'Debian') ? "/etc/mysql/my.cnf" : "/etc/my.cnf" ,
		"zarafa_mysql_cnf" => ( OS eq 'Ubuntu' or OS eq 'Debian') ? "/etc/mysql/conf.d/zarafa-innodb.cnf" : "/etc/my.cnf",
		"exim_relay_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/exim4/exim.acceptrelay" : "/etc/exim/exim.acceptrelay",
		"exim_domains_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/exim4/exim.acceptdomains" : "/etc/exim/exim.acceptdomains",
		"bbexim_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/exim4/bbexim.conf" : "/etc/exim/bbexim.conf",
		"exim_smtproute_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/exim4/exim.smtproutes" : "/etc/exim/exim.smtproutes",
		...
		"exim_blacklist" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? '/etc/exim4/exim.blacklist-hosts' : '/etc/exim/exim.blacklist-hosts',
		"exim_whitelist" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? '/etc/exim4/exim.whitelist-hosts' : '/etc/exim/exim.whitelist-hosts',
		...
		"mail2fax_config" => "/opt/yaffas/config/mail2fax/config",
		"sendfax_custom" => "/opt/yaffas/config/mail2fax/sendfax.custom",
		...
		"pdf_drivers" => ( Yaffas::Constant::OS eq 'Ubuntu' or OS eq 'Debian') ? "/opt/software/driver/gs_driver.zip" : "/data/pdf/documents/driver/gs_driver.zip",
		...
		"freshclam_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/clamav/freshclam.conf" : "/etc/freshclam.conf",
		...
		"clamd_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/clamav/clamd.conf" : "/etc/clamd.conf",
		"sa_local_conf" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/etc/spamassassin/local.cf" : "/etc/mail/spamassassin/local.cf",
...
use constant {
	APPLICATION => {
		...
		"tar" => "/bin/tar",
		"exim4" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/usr/sbin/exim4" : "/usr/sbin/exim",
		"faxinfo" => "/usr/sbin/faxinfo",
		"faxrm" => "/usr/bin/faxrm",
		"hostname" => "/bin/hostname",
		"domrename" => "/opt/yaffas/bin/domrename.pl",
		"dmesg" => "/bin/dmesg",
		"lspci" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/usr/bin/lspci" : "/sbin/lspci",
		"lshw" => ( OS eq 'Ubuntu' or OS eq 'Debian' ) ? "/usr/bin/lshw" : "/usr/sbin/lshw",
		"df" => "/bin/df",
		"rm" => "/bin/rm",
		"free" => "/usr/bin/free",
		...
		"zarafa_public_folder_script" => "/opt/yaffas/libexec/mailalias/zarafa-public-folders",
		...
		"brctl" => ((OS eq 'Ubuntu' and OSVER ne '10.04') || (OS eq 'Debian' and OSVER !~ /^6($|\.)/)) ? '/sbin/brctl' : '/usr/sbin/brctl',
