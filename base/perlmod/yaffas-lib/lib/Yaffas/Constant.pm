...
use constant {
	DIR => {
		...
		"bitkit_module" => "/opt/yaffas/lib/perl5/Yaffas/Module/",
...
use constant {
	FILE => {
		...
		"bitkit_config" => "/opt/yaffas/config/yaffas.xml",
...
#		"smb_includes_global" => ( OS eq 'Ubuntu' ? "/etc/samba/smbopts.global" : "/etc/samba/smb.conf" ),
		...
		"mybitkit_conf" => '/etc/bbportal/mybitkit.conf',
