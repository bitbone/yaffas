...
#
our $VERSION = '0.02';
our (@EXPORT, @EXPORT_OK, %EXPORT_TAGS, %Valid);
#
...
local $_; # dont overwrite $_ somewhere else.

# you can do that now more than only once.
# in new you can do it only once unless you want to use
# seek and tell every time you do a new()
# .. less memory ..
while (<DATA>) {
	chomp;
	next if /^#/;
...
Get a list of keys for a given section, use 'global' for the globl section
...
sub sectionParameter
{
    ...
    if(@_)
    {
        ...
        if($Valid{section}->{"v$version"}->{$param})
        {
            $self->{_section}->{$section}->{$param} = $value;
        }
        else
        {
            cluck("Invalid section key \"$param\" for samba version $version");
        }
    }
    return $self->{_section}->{$section}->{$param};
}

=head2 deleteSectionParameter(section, key)

This routine deletes a parameter out of a section

=cut

sub deleteSectionParameter
{
	my $self = shift;
	my $section = lc shift;
	my $param = lc shift;
	...
	if($Valid{section}->{"v$version"}->{$param})
	{
		delete $self->{_section}->{$section}->{$param};
	}
	else
	{
            cluck("Invalid section key \"$param\" for samba version $version");
...
sub deleteGlobalParameter
{
	my $self = shift;
	my $param = lc shift;
	my $version = $self->{_version};

	if($Valid{global}->{"v$version"}->{$param} || $Valid{section}->{"v$version"}->{$param})
	{
		delete $self->{_global}->{$param};
	}

	return 1;
}

=head2 deleteGlobalParameter(key)

Delete a key in the global Section
...
sub load
{
    ...
    my $isGlobal;
    my $lastSection;
    # guess version 2 we need a good way to see if version 3
    my $lastDetVersion = 2;
    local $_; # dont overwrite $_ somewhere else.
    while(<FH>)
    {
        ...
        else
        {
            ...
            if($isGlobal)
            {
                ...
                my ($v2,$v3);
                $v2 = $Valid{global}->{v2}->{$key};
                $v3 = $Valid{global}->{v3}->{$key};
                if($v3 && !$v2)
                {
                    $self->{_version} = 3;
                }
...
sub save
{
    ...
    # Ok write Header
    my @keys = $self->keys('global');
    if($#keys != -1)
    {
        print WH "[global]\n";
        foreach my $key (@keys)
        {
            $key eq "include" and next;
            print WH "\t$key = ", $self->{_global}->{$key}, "\n";
        }
        # make sure that includes are at the end
        foreach my $key (@keys)
        {
            $key eq "include" or next;
            print WH "\t$key = ", $self->{_global}->{$key}, "\n";
        }
        print WH "\n";
    }
    ...
    foreach my $skey (@sList)
    {
        ...
        {
            $subK eq "include" and next;
            print WH "\t$subK = ", $self->{_section}->{$skey}->{$subK}, "\n";
        }
        # make sure that includes are at the end
        foreach my $subK (@skList)
        {
            $subK eq "include" or next;
            print WH "\t$subK = ", $self->{_section}->{$skey}->{$subK}, "\n";
        }
        print WH "\n";
    }
...
Martin Barth (bitbone AG) martin@tmtowtdi.de info@bitbone.de
Sebastian Stumpf (bitbone AG) stumpf@bitbone.de

=head1 COPYRIGHT AND LICENSE

Copyright (C) 2004 by Salvatore ScottoDiLuzio
Copyright (C) 2005 by bitbone AG
