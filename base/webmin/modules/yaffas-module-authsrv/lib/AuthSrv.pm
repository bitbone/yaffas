...
sub check_pdc_ping()
{
	my $rv = Yaffas::do_back_quote(Yaffas::Constant::APPLICATION->{'wbinfo'}, "-p");
	if ($? == 0) {
		return $rv;
	}
	else {
...
sub check_pdc_trust()
{
	my $rv = Yaffas::do_back_quote(Yaffas::Constant::APPLICATION->{'wbinfo'}, "-t");
	if ($? == 0) {
		return $rv;
	}
	else {
...
sub mod_pam
{
	...
	Yaffas::do_back_quote($authconfig, "--".$ldap."ldapauth", "--".$winbind."winbindauth", "--update");
}
...
sub mod_nsswitch(;$)
{
	...
	unless (defined $method && $method ne "") {
		if((Yaffas::Auth::get_auth_type() eq ADS) || (Yaffas::Auth::get_auth_type() eq PDC)) {
			$method = "files winbind";
		}
		elsif ((Yaffas::Auth::get_auth_type() eq LOCAL_LDAP) || (Yaffas::Auth::get_auth_type() eq REMOTE_LDAP)) {
			$method = "files ldap";
		}
	...
	my $file = Yaffas::File::Config->new(Yaffas::Constant::FILE->{'nsswitch'},
		{
			-SplitPolicy => 'custom',
			-SplitDelimiter => '\s*:\s*',
			-StoreDelimiter => ': ',
		}
		) or throw Yaffas::Exception("err_file_write", Yaffas::Constant::FILE->{'nsswitch'});
...
sub show_pdc_users()
{
	my @rv = split(/\s+/, Yaffas::do_back_quote(Yaffas::Constant::APPLICATION->{'wbinfo'}, "-u"));
	if ($? == 0) {
		return @rv;
	}
	else {
...
sub net_rpc_join($$;$) {
	...
	if ($type eq "win") {
		...
		sleep 5;
		@re = Yaffas::do_back_quote(Yaffas::Constant::APPLICATION->{'net'}, 'ads', 'join', '-U', "$domadm%$pass" );
	}
	else {
		@re = Yaffas::do_back_quote(Yaffas::Constant::APPLICATION->{'net'}, 'join', 'ads', '-U', "$domadm%$pass" );
	}

	if ($? == 0) {
		@re = Yaffas::do_back_quote(Yaffas::Constant::APPLICATION->{'wbinfo'}, "--set-auth-user", "$domadm%$pass");
		Yaffas::Service::control(WINBIND, RESTART);
		return 1;
	}
	else {
		throw Yaffas::Exception( 'err_joining_domain', ["type = $type", "Domain Administrator = $domadm", "output: ".(join(",", @re))] );
	}
}
...
sub pam_login_winbind($) {
	my $switch = shift;
	throw Yaffas::Exception("err_only_on_off") if ($switch ne 'on' && $switch ne 'off');
	
	my $plogin = Yaffas::File->new(Yaffas::Constant::FILE->{pamd_login})
		or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{pamd_login});

	my $linenr = $plogin->search_line(qr/^\s*auth\s+sufficient\s+.*pam_winbind.so/);
	...
	$linenr = $plogin->search_line(qr/^\s*auth\s+required\s+.*pam_pwdb.so\s+use_first_pass\s+shadow\s+nullok/);
	if ( defined($linenr) && $switch eq 'off' ) {
		$plogin->splice_line($linenr, 1);
	}
	elsif (!defined($linenr) && $switch eq "on"){
		$plogin->add_line("auth\trequired\t/lib/security/pam_pwdb.so use_first_pass shadow nullok");
	}

	$linenr = $plogin->search_line(qr/^\s*account\s+required\s+.*pam_winbind.so/);
	if ( defined($linenr) && $switch eq 'off' ) {
		$plogin->splice_line($linenr, 1);
	}
	elsif (!defined($linenr) && $switch eq "on"){
...
sub set_bk_ldap_auth($$$$$$$$;$$) {
	...
	try{
		...
		$exception->add("err_searchattr") unless(Yaffas::Check::alpha_num($usersearch));
		$exception->add("err_emailattr") unless(Yaffas::Check::alpha_num($email));
		...
		my @files = (Yaffas::Constant::FILE->{pam_ldap_conf},
					 Yaffas::Constant::FILE->{libnss_ldap_conf},
					 Yaffas::Constant::FILE->{ldap_conf},
					);
		...
		{
			my $lc = Yaffas::File::Config->new($cfile,
				{
					-SplitPolicy => 'custom',
					-SplitDelimiter => '\s+',
					-StoreDelimiter => ' ',
		...
		$smb->save(Yaffas::Constant::FILE->{samba_conf}) 
			or throw Yaffas::Exception( 'err_file_write', Yaffas::Constant::FILE->{samba_conf});
		...
		my $smbldap_c = Yaffas::File->new(Yaffas::Constant::FILE->{smbldap_conf})
			or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{smbldap_conf});
		...
		if (defined($linenr))
		{
			$smbldap_c->splice_line($linenr, 1, "usersdn=\"$userdn,\${suffix}\"");
		}
		undef($linenr);
		...
		if (defined($linenr))
		{
			$smbldap_c->splice_line($linenr, 1, "groupsdn=\"$groupdn,\${suffix}\"");
		}
		undef($linenr);
		...
		my $smbldap_bind = Yaffas::File::Config->new(Yaffas::Constant::FILE->{smbldap_bind_conf},
										   {
										   -SplitPolicy => 'custom',
										   -SplitDelimiter => '\s*=\s*',
										   -StoreDelimiter => '=',
		...
		if ($host eq "127.0.0.1" || $host eq "localhost")
		{
			...
			my $conf = Yaffas::File->new(Yaffas::Constant::FILE->{hosts}) 
				or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{hosts});

			my $linenr = $conf->search_line(qr/^\d+\.\d+\.\d+\.\d+\s+.*\.$olddom.*\s+$hostname/);
			if (defined($linenr))
			{
		...
		# /etc/ldap/ldap.conf
		my $ldap_conf = Yaffas::File::Config->new(Yaffas::Constant::FILE->{'ldap_ldap_conf'}, {
												  -SplitPolicy => 'custom',
												  -SplitDelimiter => '\s+',
												  -StoreDelimiter => ' ',
												  }
...
sub get_all_sambasid ($$$$;$) {
	...
	my $msg = $ldap->bind( $binddn, password => $bindpw );
	$msg->code && throw Yaffas::Exception('err_get_sambasid', [ $msg->code, $msg->error_desc ] );
	...
	foreach my $sid ( keys %$all_sambasid ) {
		...
		$msg_dn->code && throw Yaffas::Exception('err_get_sambasid', [ $msg->code, $msg->error_desc ] );
...
sub set_local_auth(){
	...
	unless( Yaffas::Auth::get_auth_type() eq LOCAL_LDAP ){
		...
		if (-r $local) {
			...
			$file = Yaffas::File->new(Yaffas::Constant::FILE->{ldap_secret}, $pass);
			$file->save() or throw Yaffas::Exception("err_write_file", Yaffas::Constant::FILE->{ldap_secret});
			...
			$pass = $file->get_content() or throw Yaffas::Exception("err_file_read", $remote);
		}
		else {
...
sub auth_srv_pdc (;$) {
	my $auth    = shift;
	my $smbconf =
	  Yaffas::File->new( Yaffas::Constant::FILE->{smb_includes} );

	my $linenr = $smbconf->search_line(qr#^\s*include\s*=\s*/etc/samba/smbopts\.pdc-global\s*#);
	...
	if ( defined $auth ) {
		...
		if ( $auth eq 'activate' ) {
			...
			$smbconf->add_line("include = /etc/samba/smbopts.pdc-shares") unless (grep /pdc-shares$/, @content);
		}
...
sub auth_srv_ldap($)
{
	...
	$exception->add("err_auth") unless ($auth eq 'activate' ||  $auth eq 'deactivate');

	throw $exception if $exception;

	my $conf = Yaffas::File::Config->new(Yaffas::Constant::FILE->{'bkversion'}, {
										 	-SplitPolicy => 'custom',
											-SplitDelimiter => '\s*=\s*',
											-StoreDelimiter => '=',
										 }
										)
	or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{'bkversion'} );
	...
	$conf->write() or throw Yaffas::Exception("err_file_write", Yaffas::Constant::FILE->{'bkversion'} );

	# (un)set option to bind on specified device
	my $slapc = Yaffas::File->new( Yaffas::Constant::FILE->{'slapd_default_conf'} )
		or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{'slapd_default_conf'} );

	my $linenr = $slapc->search_line(qr/^\s*SLAPD_SERVICES=/);
	if (defined($linenr)) {
		if ($auth eq 'activate') {
			$slapc->splice_line($linenr, 1, "SLAPD_SERVICES=\" ldap:/// ldaps:/// \"");
		}
		else {
			$slapc->splice_line($linenr, 1, "SLAPD_SERVICES=\" ldap://127.0.0.1 \"");
		}
	}
...
sub set_pdc( ;$$$$$$$$){
	...
	try {
		...
		$exception->add( 'err_invalid_domain2' ) unless( defined($workgroup) or $workgroup eq "" );
		...
		$postfix_settings->{query_filter} = '(&(objectClass=person)(otherMailbox=%s))';
		Yaffas::Module::Mailsrv::Postfix::set_postfix_ldap($postfix_settings, "aliases");
...
sub rollback_prepare(@){
	...
	system( Yaffas::Constant::APPLICATION->{'tar'}, '--create', '--absolute-names', '--file', $tmp->filename, @checked_files);
...
sub rollback_exec( $ ){
	...
	system( Yaffas::Constant::APPLICATION->{'tar'}, '--extract', '--file', $tmp->filename, '--same-permissions', '--absolute-names', '--unlink-first' );
...
sub set_time(;$) {
	...
	if ($?) {
		$val=0;
		system(Yaffas::Constant::APPLICATION->{'logger'}, "-pwarn" , "could not connect to ntp-server $ntpserver");
	}
	else {
		system(Yaffas::Constant::APPLICATION->{'logger'}, "-pinfo" , "synchronized time with $ntpserver");
		$val=1;
	}
...
sub set_zarafa_ldap(;$) {
	...
	my $file = Yaffas::File::Config->new(Yaffas::Constant::FILE->{'zarafa_ldap_cfg'},
			...
			) or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{'zarafa_ldap_cfg'});
	my $exception = Yaffas::Exception->new();
	#set defaults
	...
	if ($type eq LOCAL_LDAP) {
		...
		my $ldap_file = Yaffas::File::Config->new(Yaffas::Constant::FILE->{'pam_ldap_conf'},
												{
												-SplitPolicy => 'custom',
												-SplitDelimiter => '\s+',
												-StoreDelimiter => ' = ',
												}
				) or throw Yaffas::Exception("err_file_read", Yaffas::Constant::FILE->{'pam_ldap_conf'});
		...
		$cfg_values->{'ldap_user_search_filter'} = "(&(objectClass=person)(objectCategory=CN=Person,CN=Schema,CN=Configuration,$rootbasedn))";
	...
	throw $exception if $exception;
	$file->write() or throw Yaffas::Exception("err_file_write", Yaffas::Constant::FILE->{'zarafa_ldap_cfg'});
...
sub test_ldaps($) {
	...
	if (Yaffas::Check::ip($dcname) || Yaffas::Check::hostname($dcname) || Yaffas::Check::domainname($dcname)) {
		system(Yaffas::Constant::APPLICATION->{ldapsearch},"-x","-b", '',"-s", "base", "-H", "ldap://$dcname");
		throw Yaffas::Exception("err_contact_host", $dcname) if ($? != 0);
		system(Yaffas::Constant::APPLICATION->{ldapsearch},"-x","-b", '',"-s", "base", "-H", "ldaps://$dcname");
		return undef if ($? != 0);
	}
	return 1;
}
...
sub set_searchldap_settings(;$) {
	...
	$exception->add("err_basedn",$ls_file->{'FILE'}) if (length($ls_ref->{BASEDN}) < 3);
	#the shortest string possible is "ldap:///" -> 10 chars
	$exception->add("err_ldapuri",$ls_file->{'FILE'}) if (length($ls_ref->{LDAPURI}) < 10);
	$exception->add("err_miss_user",$ls_file->{'FILE'}) if (length($ls_ref->{BINDDN}) < 3);
	$exception->add("err_miss_pass",$ls_file->{'FILE'}) if (length($ls_ref->{LDAPSECRET}) < 1);
	$ls_file->set_permissions("root","ldapread",0640);
	throw $exception if $exception;
	$ls_file->write() or throw Yaffas::Exception("err_file_write", Yaffas::Constant::FILE->{'ldap_settings'});
}
...
sub get_ldap_settings() {
	...
	my $ls_file = Yaffas::File::Config->new($fname,
										 {
										 -SplitPolicy => 'custom',
										 -SplitDelimiter => '\s*=\s*',
										 -StoreDelimiter => '=',
...
sub _correct_pdf_user_dir($) {
	...
	if ( -d $path) {
		system(Yaffas::Constant::APPLICATION->{'chown'}, '-R', "$name", $path);
		if($? != 0) {
