...
my $func = $self->{'unbuffered'} ? \&main::ui_print_unbuffered_header
				 : \&main::ui_print_header;
my $scripts;
...
if ($self->{'errormsg'}) {
	...
	if ($self->{'buttons'}) {
		print "<hr>\n";
		print &main::ui_buttons_start();
		foreach my $b (@{$self->{'buttons'}}) {
			print &main::ui_buttons_row(@$b);
			}
		print &main::ui_buttons_end();
		}
	}
...
&main::ui_print_footer(@footerargs);
