...
$rv .= &main::ui_form_end(\@buttonargs);
...
if (@errs) {
	my (@errlist, @vallist);
	foreach my $e (@errs) {
		push(@errlist, &main::urlize("ui_error_".$e->[0])."=".
			       &main::urlize($e->[1]));
		}
	foreach my $i ($self->list_inputs()) {
		...
		foreach $v (@vals) {
			push(@vallist,
			    &main::urlize("ui_value_".$i->get_name())."=".
			    &main::urlize($v));
			}
		}
	foreach my $h (@{$self->{'hiddens'}}) {
		push(@vallist,
		    &main::urlize($h->[0])."=".&main::urlize($h->[1]));
		}
	if ($page =~ /\?/) { $page .= "&"; }
	else { $page .= "?"; }
	&main::redirect($page.join("&", "ui_redirecting=1", @errlist, @vallist));
	exit(0);
	}
}
...
my @errs = $self->validate();
&main::error_setup($whatfailed);
if (@errs == 1) {
	&main::error($errs[0]->[2] ? "$errs[0]->[2] : $errs[0]->[1]"
				   : $errs[0]->[1]);
	}
elsif (@errs > 1) {
